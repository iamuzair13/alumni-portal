...
export default function NewslettersCard() {
  ...
  const content = useMemo(() => {
    ...
    if (!hasItems) {
      return <div className="text-sm text-gray-600 dark:text-gray-400">No newsletters yet.</div>;
    }

    return (
      <div className="grid grid-cols-1 gap-3">
        {items.map((n) => (
          <div key={n.id} className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="font-semibold text-gray-900 truncate dark:text-gray-100">{n.title || "Untitled"}</div>
                <div className="mt-1 text-xs text-gray-600 dark:text-gray-400">{n.date || formatDate(n.created_at)}</div>
              </div>
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 md:p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h4 className="text-lg font-bold text-slate-900 dark:text-gray-100">Newsletters</h4>
          <p className="text-xs text-slate-600 dark:text-gray-400">Read-only. Tap View to see all newsletters.</p>
        </div>
        <button
          type="button"
      ...
      >
        <div className="p-4 sm:p-6">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Newsletters</h3>
              <p className="text-xs text-gray-600 dark:text-gray-400">Tap View to open the newsletter link.</p>
            </div>
            <button
              type="button"
              onClick={fetchNewsletters}
              disabled={loading}
              className="inline-flex items-center justify-center rounded-lg border border-gray-300 px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
            >
              Refresh
            </button>
          </div>
