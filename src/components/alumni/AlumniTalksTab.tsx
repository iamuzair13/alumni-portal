          ...
          <Table className="min-w-full">
            <TableHeader className="bg-gradient-to-r from-slate-50 to-slate-100 sticky top-0 z-10 border-b-2 border-gray-300">
              <TableRow>
                ...
                <TableCell className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Topic</TableCell>
                <TableCell className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Day</TableCell>
                <TableCell className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Time</TableCell>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading && (
                Array.from({ length: 5 }).map((_, i) => (
                  <TableRow key={`skeleton-${i}`}>
                    ...
                    <TableCell className="px-6 py-4"><div className="h-5 w-40 bg-gray-200 animate-pulse rounded" /></TableCell>
                    <TableCell className="px-6 py-4"><div className="h-5 w-20 bg-gray-200 animate-pulse rounded" /></TableCell>
                    <TableCell className="px-6 py-4"><div className="h-5 w-20 bg-gray-200 animate-pulse rounded" /></TableCell>
                  </TableRow>
                ))
              )}
              ...
              {!isLoading && !isError && pageItems.map((item, idx) => (
                <TableRow key={`${item.sapid}-${idx}`} className="odd:bg-white even:bg-gray-50/50 hover:bg-blue-50/50">
                  ...
                  <TableCell className="px-6 py-4 text-sm text-slate-700">{item.day || "-"}</TableCell>
                  <TableCell className="px-6 py-4 text-sm text-slate-700">{item.time || "-"}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
