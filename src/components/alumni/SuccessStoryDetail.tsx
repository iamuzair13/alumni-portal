...
export type SuccessStoryDetailData = {
  ...
  criteriaResponses?: { criterion_id: number; label: string; response: string }[];
};
...
export default function SuccessStoryDetail({
  ...
  const hasCriteria =
    (story.criteriaResponses && story.criteriaResponses.length > 0) ||
  ...
  const achievementsDisplay =
    ...
    story.criteriaResponses?.find((r) => /achievements?/i.test(r.label))?.response?.trim();
    ...
    <div className="min-w-0 flex-1">
      ...
      {achievementsDisplay && (
        <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-yellow-300/40 bg-yellow-400/10 px-3 py-1.5 backdrop-blur-sm">
          ...
          <span className="text-sm font-semibold text-yellow-100">{achievementsDisplay}</span>
        </div>
      )}
        ...
        <div className="border-b border-gray-200 bg-slate-50 px-6 py-6 md:px-10 dark:border-gray-700 dark:bg-gray-800/40">
          ...
          {!hasCriteria ? (
            ...
            <dl className="mt-4 space-y-4 text-sm">
              {story.criteriaResponses && story.criteriaResponses.length > 0 ? (
                story.criteriaResponses.map((r) => (
                  <div key={r.criterion_id}>
                    <dt className="font-medium text-gray-700 dark:text-gray-300">{r.label}</dt>
                    <dd className="mt-1 text-gray-900 dark:text-gray-100">
                      {criteriaValue(r.response)}
                    </dd>
                  </div>
                ))
              ) : (
