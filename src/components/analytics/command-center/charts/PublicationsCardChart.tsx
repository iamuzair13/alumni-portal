...
export function PublicationsCardChart({
  ...
  if (stories === 0 && storiesQuarter === 0 && newsletters === 0 && surveys === 0) {
  ...
  return (
    <div className="grid h-full w-full grid-cols-2 gap-2">
      <MetricTile label="Success stories" value={stories} accent="violet" delay={0.05} />
      <MetricTile label="Newsletters" value={newsletters} accent="indigo" delay={0.1} />
      <div className="flex min-h-0 flex-col justify-center rounded-xl border border-sky-200/70 bg-gradient-to-br from-sky-50/90 to-white px-2.5 py-2 dark:border-sky-500/20 dark:from-sky-500/10 dark:to-gray-900/40">
        <p className="text-[9px] font-semibold uppercase tracking-wider text-sky-600 dark:text-sky-400">
          Stories this Q
        </p>
        <p className="mt-1 text-lg font-bold tabular-nums leading-none text-sky-700 dark:text-sky-300">
          {storiesQuarter.toLocaleString()}
        </p>
        <p className="mt-1 truncate text-[8px] font-medium text-slate-400 dark:text-slate-500" title={quarterLabel}>
          {shortQuarter}
        </p>
      </div>
      <MetricTile label="Surveys" value={surveys} accent="amber" delay={0.18} />
    </div>
  );
}
