...
export function MembershipsCardChart({
  ...
  if (total === 0 && gym === 0 && pool === 0 && qalander === 0) {
    return <ChartEmpty height={100} variant="premium" message="No memberships yet" />;
  }

  return (
    <div className="grid h-full w-full grid-cols-2 gap-2">
      <MetricTile label="Total memberships" value={total} accent="violet" delay={0.05} />
      <MetricTile label="Gym" value={gym} accent="indigo" delay={0.1} />
      <MetricTile label="Pool" value={pool} accent="sky" delay={0.14} />
      <MetricTile label="Qalandars club" value={qalander} accent="amber" delay={0.18} />
    </div>
  );
}
