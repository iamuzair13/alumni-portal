...
function RegionBarRow({
  ...
  return (
    <div
      className={`grid h-full min-h-0 grid-cols-[1.375rem_minmax(0,1fr)_1.75rem] items-center gap-1 rounded-md px-0.5 ${
        isLeader && bar.count > 0 ? "bg-emerald-50/60 dark:bg-emerald-500/[0.06]" : ""
      }`}
      title={`${bar.label}: ${bar.count.toLocaleString()}`}
    >
      <span
        className="flex h-4 w-[1.375rem] shrink-0 items-center justify-center rounded text-[7px] font-bold leading-none"
        style={{
          color: bar.color,
...
export function LocationCardChart({
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col gap-1">
      ...
      >
        {allBars.map((bar, i) => {
          const segment = total > 0 ? (bar.count / total) * 100 : 0;
