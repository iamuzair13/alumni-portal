...
import { ccAccent, ccCard, ccCardSub, ccCardTitle, ccCardValueLg, ccCardValueMd } from "./theme";
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-1">
        <div className="shrink-0">
          <div className="flex items-baseline gap-2">
            <span className={compact ? ccCardValueMd : ccCardValueLg}>{displayValue}</span>
            {trend ? (
              <span
          ...
          {secondaryLabel ? (
            <p className={`mt-0.5 line-clamp-1 ${ccCardSub}`}>{secondaryLabel}</p>
          ) : null}
        </div>
        {chart ? (
          <div
            className={`w-full max-w-full overflow-hidden ${
              chartFill ? "min-h-0 flex-1 pt-1" : "shrink-0 pt-1"
            }`}
          >
            {chart}
          </div>
