...
export function TrainedAdminsExpandPanel({
  ...
  const tableRows = useMemo(() => {
    const body = admins.byFaculty.map((row) => ({
      ...
      total: (row.admins + row.viewers).toLocaleString(),
      firstTrained: formatTrainedDate(row.firstTrainedAt),
      lastTrained: formatTrainedDate(row.lastTrainedAt),
    }));
    if (!body.length) return body;
    const facultyScopedTotal = admins.adminsTotal + admins.viewersTotal;
    return [
      ...body,
      {
        faculty: "Total (distinct users)",
        admins: admins.adminsTotal.toLocaleString(),
        viewers: admins.viewersTotal.toLocaleString(),
  ...
  const kpis = [
    {
      ...
      value: admins.total,
      sub: "Superadmins + admins + viewers",
    ...
    },
    {
      ...
      value: admins.adminsTotal,
      sub: "Distinct users with type admin",
      color: KPI_COLOR_HEX.indigo,
    },
    {
      label: "Faculty viewers",
      value: admins.viewersTotal,
      sub: "Distinct users with type viewer or user",
  ...
  return (
    <div className="space-y-5">
      <p className="text-xs text-gray-500 dark:text-gray-400">
        ...
        faculty rows. Faculty table counts distinct users per faculty; row sums may exceed headline totals
        when one user is assigned to multiple faculties.
      </p>
