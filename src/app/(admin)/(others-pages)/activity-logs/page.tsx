...
export default function ActivityLogsPage() {
  ...
  const [data, setData] = useState<LogsResponse>({ items: [], total: 0, limit: 50, offset: 0 });
  ...
  }, [isSuperAdmin, action, actorUserId, q, from, to, limit, page]);

  useEffect(() => {
    setPage(1);
  }, [action, actorUserId, q, from, to, limit]);
  ...
  return (
    <ComponentCard title="Activity Logs" desc="Track actions performed by Admin and Viewer accounts.">
      <div className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="text-sm text-gray-600 dark:text-gray-400">
            {loading ? "Loading..." : `Showing ${data.items.length.toLocaleString()} of ${data.total.toLocaleString()}`}
          </div>
