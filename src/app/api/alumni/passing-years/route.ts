...
export async function GET(req: Request) {
  try {
    ...
    const masterFilterConditions = buildMasterFilterConditions(searchParams, "passingYear");
    ...
    const rows = await sql/* sql */`
      WITH year_data AS (
        SELECT 
          CASE 
            WHEN a.yearofending IS NULL 
            THEN 'Null'
            ELSE a.yearofending::text
          END as year_value,
          a.yearofending as year_raw,
          COUNT(*) as count
